...
import { fetchTTSLateReportingDataAction, fetchTTSLateReportingTableGrid1Action, fetchTTSLateReportingTableGrid2Action } from '../components/redux/reducers/TTLateReportingSlice'

const TTLateReporting = () => {
    ...
    const { ttsLateReportingdata } = useSelector((state => state.fetchTTSLateReportingData));
    const { ttsLateReportingtablegrid1data } = useSelector((state => state.fetchTTSLateReportingTableGrid1));
    const { ttsLateReportingtablegrid2data } = useSelector((state => state.fetchTTSLateReportingTableGrid2));
    ...
    useEffect(() => {
        if (token && userId && roleId && startDate && endDate) {
            dispatch(fetchTTSLateReportingDataAction({ token, userId, roleId, startDate, endDate }));
            dispatch(fetchTTSLateReportingTableGrid1Action({ token, userId, roleId, startDate, endDate }));
            dispatch(fetchTTSLateReportingTableGrid2Action({ token, userId, roleId, startDate, endDate }));
        }
    ...
    const summaryData = [
        { label: 'Total Indent', value: ttsLateReportingdata?.Data?.[0]?.TotalIndentCount || 0, color: '#FAD2D2' },
        { label: 'Late Result Count', value: ttsLateReportingdata?.Data?.[0]?.TotalLateCount || 0, color: '#D4EDDA' },
        { label: 'Percentage', value: ` ${ttsLateReportingdata?.Data?.[0]?.LatePercentage || 0}%`, color: '#D1ECF1' },
        { label: 'Lower Value', value: `15`, color: '#D1ECF1' },
        { label: 'Higher Value', value: `45`, color: '#D1ECF1' },
    ];

    const tableData = ttsLateReportingtablegrid1data ? ttsLateReportingtablegrid1data.Data : [];

    const logData = ttsLateReportingtablegrid2data ? ttsLateReportingtablegrid2data.Data : [];
    ...
    return (
        <View style={styles.container}>
           <View style={styles.filterContainer}>
            ...
            <Text style={{ fontWeight: 'bold', fontSize: 10, marginLeft: 50 }}>Cycle Time From: Card Issue Time To : Lie Entry Gate Time</Text>
            ...
            <ScrollView>
                <Text style={styles.header}>Truck Wise Late Reporting</Text>
                <FlatList
                    ...
                    ListHeaderComponent={
                        <View style={styles.tableHeader}>
                            ...
                            <Text style={styles.headerText}>Late Count</Text>
                    ...
                    renderItem={({ item }) => (
                        <View style={styles.row}>
                            ...
                            <Text style={styles.cell}>{item.TotalIndentCount}</Text>
                            <Text style={styles.cell}>{item.TotalLateCount}</Text>
                            <Text style={styles.cell}>{item.LatePercentage}%</Text>
                        </View>
                    )}
                />
                ...
                <Text style={styles.header}>Bay Wise TTS LateReporting</Text>
                <FlatList
                    ...
                    ListHeaderComponent={
                        <View style={styles.tableHeader}>
                            <Text style={styles.headerText}>Bay Number</Text>
                            <Text style={styles.headerText}>Indent Count</Text>
                            <Text style={styles.headerText}>Late Count</Text>
                            <Text style={styles.headerText}>Percentage</Text>
                        </View>
                    ...
                    renderItem={({ item }) => (
                        <View style={styles.row}>
                            <Text style={styles.cell}>{item.BayNumber}</Text>
                            <Text style={styles.cell}>{item.TotalIndentCount}</Text>
                            <Text style={styles.cell}>{item.TotalLateCount}</Text>
                            <Text style={styles.cell}>{item.LatePercentage}%</Text>
                        </View>
...
const styles = StyleSheet.create({
    ...
    summaryContainer: { flexDirection: 'row', justifyContent: 'space-around', marginBottom: 10, marginTop: 10 },
    summaryCard: { width: 65, height: 60, borderRadius: 10, alignItems: 'center', justifyContent: 'center', padding: 5, elevation: 3 },
...
export default TTLateReporting;
